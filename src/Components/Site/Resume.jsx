...
function SectionTitle({ children }) {
  return (
    <h2 className="text-2xl sm:text-3xl font-bold mb-8 bg-gradient-to-r from-white to-[#a78bfa] bg-clip-text text-transparent">
      {children}
    </h2>
  );
}
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-24">
          ...
          >
            <div>
              ...
              >
                Geraldo Chin
                <span className="block text-xl sm:text-2xl font-normal mt-2 bg-gradient-to-r from-white to-[#a78bfa] bg-clip-text text-transparent">
                  Fullstack Developer
                </span>
              </motion.h1>
            ...
            >
              ...
              <div className="relative flex flex-col gap-0">
                ...
                {experience.map((exp, i) => (
                  <motion.div key={i} variants={fadeUp} className="group relative pl-12 pb-10 last:pb-0">
                    <div className="absolute left-0 top-0 w-9 h-9 rounded-full border border-purple-900 bg-gray-950 flex items-center justify-center text-[#a78bfa] text-sm group-hover:border-[#a78bfa]/50 transition-colors">
                      <FaBriefcase />
                    </div>

                    <div className="bg-gray-900/50 border border-purple-900/30 rounded-2xl p-6 hover:border-[#a78bfa]/30 transition-all duration-300 group-hover:bg-gray-900/80">
                      <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                        <div>
                          <h3 className="text-white font-semibold text-base">{exp.role}</h3>
                          <p className="text-[#a78bfa] text-sm font-medium">{exp.company}</p>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.tags.map(t => (
                          <span key={t} className="text-[11px] text-[#a78bfa] bg-[#a78bfa]/10 border border-[#a78bfa]/20 px-2.5 py-0.5 rounded-full">
                            {t}
                          </span>
                        ))}
                      </div>
                ...
                <div className="relative flex flex-col gap-0">
                  ...
                  {education.map((edu, i) => (
                    <motion.div key={i} variants={fadeUp} className="group relative pl-12 pb-8 last:pb-0">
                      ...
                      <div className="bg-gray-900/50 border border-purple-900/30 rounded-2xl p-5 hover:border-[#a78bfa]/30 transition-all duration-300">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <h3 className="text-white font-semibold text-sm leading-snug">{edu.degree}</h3>
                          <span className="text-[10px] text-[#a78bfa] uppercase tracking-widest bg-[#a78bfa]/10 px-2 py-0.5 rounded-full border border-[#a78bfa]/20 flex-shrink-0">
                            {edu.period}
                          </span>
                        </div>
                        <p className="text-[#a78bfa] text-xs font-medium mb-2">{edu.school}</p>
                ...
                <div className="flex flex-col gap-3">
                  {certifications.map((cert, i) => (
                    <motion.div
                      ...
                      variants={fadeUp}
                      whileHover={{ x: 4 }}
                      className="group flex items-center justify-between gap-4 bg-gray-900/50 border border-purple-900/30 rounded-xl px-5 py-4 hover:border-[#a78bfa]/40 transition-all cursor-default"
                    >
                      <div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {skillGroups.map((group, i) => (
                <motion.div
                  ...
                  variants={fadeUp}
                  whileHover={{ y: -4 }}
                  className="group bg-gray-900/50 border border-purple-900/30 rounded-2xl p-6 hover:border-[#a78bfa]/40 transition-all duration-300"
                >
                  <div className="flex items-center gap-3 mb-5">
                    <div className="w-9 h-9 rounded-xl bg-[#a78bfa]/10 border border-[#a78bfa]/20 flex items-center justify-center text-[#a78bfa] text-sm group-hover:bg-[#a78bfa]/20 transition-colors">
                      {group.icon}
                    </div>
            ...
            <div className="relative flex flex-col sm:flex-row items-center justify-between gap-6 p-10">
              ...
              <div className="flex gap-3 flex-shrink-0">
                <a
                  href="/cv.pdf"
                  download
                  className="flex items-center gap-2 border border-[#a78bfa]/30 hover:border-[#a78bfa] text-gray-300 hover:text-white px-5 py-2.5 rounded-full text-sm font-medium transition-all"
                >
                  <FaDownload size={11} />
                  CV em PDF
                </a>
                <a
                  href="#contact"
                  className="flex items-center gap-2 bg-gradient-to-r from-[#a78bfa] to-[#6b3ff0] hover:opacity-90 px-6 py-2.5 rounded-full text-sm font-semibold transition-all hover:shadow-[0_0_24px_rgba(167,139,250,0.35)]"
                >
                  Contactar
                </a>
              </div>
